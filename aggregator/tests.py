...
class ModelTests(django.test.TestCase):
    ...
    def _tag():
        tg = str(uuid.uuid4())
        t = aggregator.models.Tag(tag=tg)
        t.save()
        return t.id, tg
    ...
    def _event():
        ex = str(uuid.uuid4())
        ty = aggregator.models.EventType.other
        tl = str(uuid.uuid4())
        dc = str(uuid.uuid4()) * 5
    ...
    def _city():
        nm = str(uuid.uuid4())
        c = aggregator.models.City(name=nm)
        c.save()
        return c.id, nm
    ...
    def _place(cls):
        ex = str(uuid.uuid4())
        ty = aggregator.models.PlaceType.other
        tl = str(uuid.uuid4())
        tt = str(uuid.uuid4()) * 10
        ur = str(uuid.uuid4()) * 2
        cid, _ = cls._city()
        ct = aggregator.models.City.objects.get(id=cid)
        ad = str(uuid.uuid4()) * 4
    ...
    def _schedule(cls):
        ...
        e = aggregator.models.Event.objects.get(id=eid)
        pid, _, _, _, _, _, _, _, _, _ = cls._place()
        p = aggregator.models.Place.objects.get(id=pid)
        dt = cls._random_date()
        st = cls._random_time()
        et = cls._random_time()
    ...
    def test__tag(self):
        tid, tg = self._tag()
        t = aggregator.models.Tag.objects.get(id=tid)
    ...
    def test__event_tags(self):
        ...
        et = aggregator.models.EventTags.objects.get(event_id=eid, tag_id=tid)
    ...
    def test__event_images(self):
        ...
        ei = aggregator.models.EventImages.objects.get(event_id=eid)
    ...
    def test__event_data(self):
        ...
        ed = aggregator.models.EventData.objects.get(event_id=eid, key=k)
        self.assertEqual(ed.key, k)
        self.assertEqual(ed.value, v)

    def test__city(self):
        cid, nm = self._city()
        c = aggregator.models.City.objects.get(id=cid)
    ...
    def test__place_tags(self):
        ...
        pt = aggregator.models.PlaceTags.objects.get(place_id=pid, tag_id=tid)
        self.assertEqual(pt.place_id, pid)
        self.assertEqual(pt.tag.tag, t.tag)

    def test__place_phones(self):
        pid, _, _, _, _, _, _, _, _, _ = self._place()
        pt = aggregator.models.PhoneType.other
        ph = str(uuid.uuid4())
        p = aggregator.models.Place.objects.get(id=pid)
        p.placephones_set.create(type=pt, phone=ph)
        pp = aggregator.models.PlacePhones.objects.get(place_id=pid, type=pt)
    ...
    def test__place_metros(self):
        ...
        pm = aggregator.models.PlaceMetros.objects.get(place_id=pid)
        self.assertEqual(pm.metro, ms)

    def test__place_work_times(self):
        pid, _, _, _, _, _, _, _, _, _ = self._place()
        ty = aggregator.models.WorkTimeType.openhours
        wt = str(uuid.uuid4()) * 3
        p = aggregator.models.Place.objects.get(id=pid)
        p.placeworktimes_set.create(type=ty, work_time=wt)
        pw = aggregator.models.PlaceWorkTimes.objects.get(
            place_id=pid, type=ty)
    ...
    def test__place_images(self):
        ...
        pi = aggregator.models.PlaceImages.objects.get(place_id=pid)
